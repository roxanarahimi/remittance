...
//        where('CreationDate', '>=', today()->subDays(2))->
        pluck('ProductNumber');
        $products = Product::where('Name', 'like', '%نودالیت%')->whereNotIn('ProductNumber', $productIds)->get();
...
//            where('DeliveryDate', '>=', today()->subDays(2))->
            ...
            foreach ($d1 as $item) {
                ...
                foreach ($item->OrderItems as $item2) {
                    ...
                    $product = InvoiceProduct::where('ProductNumber', $item2->product->ProductNember)->first();
                    if (!$product) {
                        InvoiceProduct::create([
